...
export class YaneComponent extends THREE.Group {
	...
	constructor(PARAMS, A, B, C, D, direction, tarukiInterval) {
		...
		this.localVertices = {A:A.clone(), B:B.clone(), C:C.clone(), D:D.clone()};
	...
	calcParameters() {
		...
		this.sumimunePow = this.PARAMS.yane.sumimunePow;
	...
	calcUpperVertex(bottom) {
		...
		const Ycoordinate = this.sei + (this.C.y - this.sei) * Math.pow(changeZ / changeLevel, this.sumimunePow);
...
class YaneBody extends THREE.Group {
	...
	generateLine() {
		...
		for (let i = 0; i < this.vertices.upper.length; i++) {
			upperPoints.push(this.vertices.upper[i]);
			lowerPoints.push(this.vertices.lower[i]);
		}

		this.upperLine = new ModelingSupporter().generateLineMesh(upperPoints);
		this.add(this.upperLine);

		this.lowerLine = new ModelingSupporter().generateLineMesh(lowerPoints);
